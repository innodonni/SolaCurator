...
print "[RANDOM_MOVE daemon for ", (the) self ,"]^";
              ...
              objectloop (i in compass)
                if (NPCLeadsTo(i,parent(self),ANY_PATH))
                {  n++;
#ifdef DEBUG;
if (parser_trace>1)
...
#endif;
              objectloop (i in compass)
              { if (NPCLeadsTo(i,parent(self),ANY_PATH)) n++;
                if (n==k)
                {  MoveNPCDir(self,i);
                   break;
                }
...
Message "** MoveClass::NPCLeadsTo assuming quiet *_to (NOISY_DIR_TOS not defined) **";
Endif;

[ NPCLeadsTo direction thisroom path_type k tmp tmp2;
#ifdef DEBUG;
   if (parser_trace>2)
     print "[NPCLeadsTo ", (name)direction, " ", (name)thisroom, "]^";
#endif;
...
[ MoveNpcDir npc direction i j p message;
  message=2;
  p=parent(npc);
  i=NPCLeadsTo(direction,p, ANY_PATH);
  if (i==0)
  { npc.npc_blocked();
#ifdef DEBUG;
if (parser_trace>1)
